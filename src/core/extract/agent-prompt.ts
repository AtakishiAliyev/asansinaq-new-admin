...
- Every picture the question keeps — its figure and every picture option — is REGENERATED. There is no way to keep a piece of the page: the source books are watermarked, the bank is meant to outlive them, and a saved crop carries someone else's mark forever. Two ways to make a picture, and no third:
  ...
  \`generate\` — hand the region to the image model. Use it for everything a pen cannot: shaded objects, rendered illustrations, textures, three-dimensional drawings. A question with five picture options needs five separate calls, and that is expected — do it.
  If a picture will not come out right either way, the question does not get saved: call \`give_up\` and say which picture defeated you and what the tools reported. A missing question costs one question. A watermarked one costs the bank its licence to exist.
