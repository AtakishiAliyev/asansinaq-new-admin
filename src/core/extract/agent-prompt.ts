...
- Every picture the question keeps — its figure and every picture option — is REGENERATED. It is not cut out of the page. The source books are watermarked and the bank is meant to outlive them, so a saved crop carries someone else's mark forever and a saved regeneration does not. Two ways to regenerate:
  ...
  \`cut\` is refused until a draw or a generate on that same region has actually been tried and actually failed. "The region is clean" is not a reason — a clean region is exactly what regenerates well. Cutting keeps the page's watermark and its printed option letters, both of which stay in the bank for good.
